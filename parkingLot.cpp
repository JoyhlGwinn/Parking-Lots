...
// Update the parking lot data to the text file
void ParkingLot::updateFile(const string& filename) const{
    ofstream file(filename);
    if (file.is_open()) {
        for (int i = 0; i < lotCapacity; ++i) {
            if (parkedCars[i] != nullptr) {
                file << "Car Make: " << parkedCars[i]->getMake() << "\n"
                     << "Car Model: " << parkedCars[i]->getModel() << "\n"
                     << "License Plate: " << parkedCars[i]->getLicensePlate() << "\n"
                     << "Color: " << parkedCars[i]->getColor() << "\n"
                     << "Assigned Spot: " << i + 1 << "\n"
                     << "-----------------------\n";
            }
        }
